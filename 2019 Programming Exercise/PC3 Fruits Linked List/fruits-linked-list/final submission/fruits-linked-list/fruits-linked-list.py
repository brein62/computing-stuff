...
class LinkedList:
    def __init__(self, size = 30):                          # Initialise()
        self.__Node = [ListNode() for i in range(size + 1)]
        self.__Start = 0
        self.__NextFree = 1
        for i in range(1, 30):
    ...
    def AddNode(self):
        if self.IsFull():
            print("Linked list is full, cannot insert!")
        else:
    ...
    def __TraversalInReverseOrder(self, index):
        if index != 0:
            self.__TraversalInReverseOrder(self.__Node[index].getPointer())     # call stack <3
            print(self.__Node[index].getData())
